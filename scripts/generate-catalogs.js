...
function renderGrid(){const grid=document.getElementById("grid");const header=\`<div class="row header-row"><div class="cell label">&nbsp;</div>\${domains.map(d=>\`<div class="cell label">\${d.g}<br><span>\${d.short}</span></div>\`).join("")}</div>\`;const rows=[1,2,3,4,5,6,7].map(p=>{let cells=[\`<div class="cell period-label">Period \${p}</div>\`];for(let g=1;g<=18;g++){if(p===6&&g===3){cells.push('<div class="cell pointer"><div class="range">57-71</div><div class="ptr-label">Work IQ</div></div>');continue;}if(p===7&&g===3){cells.push('<div class="cell pointer"><div class="range">89-103</div><div class="ptr-label">Governed Actions</div></div>');continue;}const a=agents.find(x=>x.group===g&&x.period===p);cells.push(a?cellHtml(a):'<div class="cell empty"></div>');}return \`<div class="row">\${cells.join("")}</div>\`;}).join("");const core=bandRow("Work IQ","Context substrate",agents.filter(a=>a.group==="L"));const trust=bandRow("Governed Actions","Action substrate",agents.filter(a=>a.group==="A"));grid.innerHTML=header+rows+\`<div class="substrate-bands">\${core}\${trust}</div>\`;document.querySelectorAll(".cell[data-symbol]").forEach(el=>{el.addEventListener("click",()=>openDetail(el.dataset.id));el.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" ")openDetail(el.dataset.id);});});}
